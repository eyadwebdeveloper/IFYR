...
document.getElementById('continue-google').addEventListener('click', (event)=>{
  ...
  signInWithPopup(auth, provider)
    ...
    .catch((error) => {
      showMsg('Error signing in please reload and try again');
    });
})  
...
resetForm.addEventListener('submit', (event)=>{
  ...
  .catch((error) => {
    showMsg("Error sending password reset link please reload and try again");
  });
})
